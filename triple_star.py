...
def triple_star_IO_Generator(prime, error_disk):
    ...
    for i in range(1, prime):
        ...
        error_block_position = triple_star_position(i, error_disk, prime)
...
def triple_star_method0(position, p):
    (x, y) = triple_star_xy_decoder(position, p)
    sequence = []
    for j in range(0, p):
        if j != y:
            block_position = triple_star_position(x, j, p)
...
def triple_star_method1(position, p):
    ...
    for j in range(p):
        if j!=y and (i+j)%p != 0:
            block_position=triple_star_position((i+j)%p, j, p)
            sequence.append(block_position)

    # the position of the parity
    if i!=0:
        block_position=triple_star_position(i, p, p)
        sequence.append(block_position)
...
def triple_star_method2(position, p):
    ...
    # the position of the parity
    if i!=0:
        block_position = triple_star_position(i, p+1, p)
        sequence.append(block_position)

    return set(sequence)


def triple_star_position(x, y, p):
    # x=x-1 in position calculation
    x=x-1
    return x * (p + 2) + y


def triple_star_xy_decoder(position, p):
    # x=x+1 in equation
    x = int(position / (p + 2))+1
    y = position % (p + 2)
    return x, y